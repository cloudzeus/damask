...
describe('permissions.ts derives losslessly from the registry', () => {
  const EXPECTED_KEYS = [
    ...
    'regions.view','regions.manage','kad.view','kad.manage','mail.report',
  ]
  it('exposes exactly the original 27 permission keys, plus taxform.manage/taxform.scan/programs.manage/pm.manage/pm.work/regions.view+manage/kad.view+manage/mail.report', () => {
    expect(new Set(CATALOG.map(p => p.key))).toEqual(new Set(EXPECTED_KEYS))
    expect(CATALOG.length).toBe(EXPECTED_KEYS.length)
  })
})

describe('mail-report registry item', () => {
  it('is registered with its own permission', () => {
    const item = allItems().find(i => i.key === 'mail-report')
    expect(item?.href).toBe('/mail-report')
    expect(item?.menuPermission).toBe('mail.report')
    expect(item?.permissions.map(p => p.key)).toEqual(['mail.report'])
  })
})
