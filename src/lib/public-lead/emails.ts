  ...
  const html = renderEmailShell({
    ...
    bodyHtml: `
      ...
      <div style="text-align:center;margin:8px 0 18px;">
        <span style="display:inline-block;font-size:32px;font-weight:700;letter-spacing:0.32em;color:#16323F;background:#EEF4F5;border:1px solid #DCE5E9;border-radius:12px;padding:14px 22px 14px 30px;">${code}</span>
      </div>
...
export function teamNewLeadEmail(input: {
  ...
  const table = rows
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 10px 6px 0;color:#8098A5;font-size:12.5px;white-space:nowrap;">${escapeHtml(k)}</td><td style="padding:6px 0;color:#16323F;font-size:13.5px;font-weight:600;">${escapeHtml(v)}</td></tr>`,
    )
    .join('')
