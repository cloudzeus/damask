      ...
      <div className="stagger mb-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {kpis.map(k => (
          <div key={k.title} className="glass lift relative px-[17px] pt-[15px] pb-[13px]">
            ...
            <div className="text-[0.71875rem] font-bold text-muted-foreground">{k.title}</div>
            <div className="mt-[3px] text-[2.0625rem] leading-none font-[250] tracking-[-0.015em] tabular-nums">{k.value}</div>
            <div className="mt-1 flex items-center justify-between gap-2">
              <span className="rounded-full px-2 py-0.5 text-[0.65625rem] font-extrabold" style={{ color: k.color, background: k.bg }}>{k.hint}</span>
            </div>
          </div>
        ))}
      </div>
